...
export default function Index() {
  ...
  const scenarios = useMemo(() => {
    ...
    // Conservador: -20% em taxas, +20% em CPL, -20% Ticket
    const conservador = calculateROI(
      inv, 
      cpl * 1.2, 
      Math.min(100, tp * 1.2), 
      tq * 0.8, 
    ...
    // Otimista: +50% em taxas, -50% em CPL, +50% Ticket
    const otimista = calculateROI(
      inv, 
      cpl * 0.5, 
      tp * 0.5, 
      Math.min(100, tq * 1.5), 
  ...
  return (
    <div className="min-h-screen bg-black grid-bg scanline-overlay relative overflow-x-hidden">
              ...
              <svg width="80" height="80" viewBox="0 0 72 72">
                <defs>
                  <linearGradient id="vGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stopColor="#39FF14" /><stop offset="100%" stopColor="#2ecc71" />
                  </linearGradient>
                </defs>
                <polygon points="36,2 66,19 66,53 36,70 6,53 6,19" fill="none" stroke="#39FF14" strokeWidth="1.5" opacity="0.3" />
                <path d="M20,20 L36,52 L52,20" fill="none" stroke="url(#vGrad)" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold tracking-[0.3em] text-white uppercase mb-4" style={{ fontFamily: "'Orbitron', monospace" }}>
              VANGUARD
            </h1>
            <p className="text-[#39FF14] text-sm md:text-xl tracking-[0.5em] uppercase font-light">
              Inteligência Financeira de ROI
            </p>
          </header>
        }
      >
      ...
      {/* Main Content for PDF Capture */}
      <div id="pdf-content" className={`max-w-6xl mx-auto px-6 pb-24 ${isExporting ? 'print-mode bg-black' : ''}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          {/* Left Column: Inputs (Hidden in Export) */}
          <div className="lg:col-span-4 space-y-6 print:hidden">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-[#39FF14] neon-glow" />
              <span className="text-[10px] tracking-[0.3em] uppercase text-[#A8A9AD]">Configuração</span>
            </div>
            <div className="bg-[#050505] p-6 rounded-2xl border border-[#1A1A1A] space-y-6">
              <GlowInput label="Investimento" value={investimento} onChange={setInvestimento} prefix="R$" icon={<DollarIcon />} />
              <GlowInput label="Custo Lead (CPL)" value={custoLead} onChange={setCustoLead} prefix="R$" icon={<ChatIcon />} />
              <GlowInput label="Taxa Perda (Não Responde)" value={taxaPerda} onChange={setTaxaPerda} suffix="%" icon={<LossIcon />} />
              <GlowInput label="Taxa Qualificação" value={taxaQualificacao} onChange={setTaxaQualificacao} suffix="%" icon={<UserCheckIcon />} />
              <GlowInput label="Taxa Agendamento" value={taxaAgendamento} onChange={setTaxaAgendamento} suffix="%" icon={<CalendarIcon />} />
              <GlowInput label="Taxa Fechamento" value={taxaFechamento} onChange={setTaxaFechamento} suffix="%" icon={<CheckIcon />} />
              <GlowInput label="Ticket Médio" value={ticketMedio} onChange={setTicketMedio} prefix="R$" icon={<TagIcon />} />
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="flex items-center gap-2 mb-2">
              <div className="w-2 h-2 rounded-full bg-[#39FF14] neon-glow" />
              <span className="text-[10px] tracking-[0.3em] uppercase text-[#A8A9AD]">Desempenho Atual</span>
            </div>
            ...
            <div className="p-8 rounded-2xl bg-[#050505] border border-[#1A1A1A]">
              <p className="text-[10px] tracking-[0.3em] uppercase text-[#A8A9AD] mb-6 font-bold">Funil de Conversão Detalhado</p>
        ...
        {/* Scenarios Section: Side-by-Side Blocks */}
        <div className="mt-16">
          <div className="flex items-center gap-2 mb-8">
            <div className="w-2 h-2 rounded-full bg-[#39FF14] neon-glow" />
            <span className="text-[10px] tracking-[0.3em] uppercase text-[#A8A9AD]">Análise de Cenários Comparativos</span>
          </div>
        ...
        {/* Footer for PDF */}
        <div className="hidden print-only:flex justify-between items-center mt-16 pt-8 border-t border-[#1A1A1A]">
          <p className="text-[#555] text-[10px] uppercase tracking-widest">VANGUARD COMPANY • RELATÓRIO DE INTELIGÊNCIA COMERCIAL</p>
          <p className="text-[#555] text-[10px]">{new Date().toLocaleDateString("pt-BR")}</p>
        </div>
      </div>
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 print:hidden z-50">
        <button onClick={handleExportPDF} disabled={isExporting}
          className="flex items-center justify-center gap-3 bg-[#39FF14] hover:bg-[#45ff26] text-black font-bold py-4 px-8 rounded-2xl transition-all duration-300 shadow-[0_0_30px_rgba(57,255,20,0.3)] active:scale-95 uppercase tracking-widest text-xs disabled:opacity-50">
          {isExporting ? "Gerando..." : <><PDFIcon /> Exportar Relatório PDF</>}
        </button>
        <Link href="/forecasting" className="flex items-center justify-center gap-3 bg-black border border-[#39FF14]/30 text-[#39FF14] hover:bg-[#39FF14]/10 font-bold py-4 px-8 rounded-2xl transition-all duration-300 uppercase tracking-widest text-xs">
          Ir para Previsão Anual
        </Link>
      </div>

      <style dangerouslySetInnerHTML={{ __html: `
        .print-mode { width: 1400px !important; max-width: none !important; margin: 0 !important; }
        @media print { .print-hidden { display: none !important; } .print-only { display: block !important; } }
      `}} />
    </div>
  );
}
...
// ─── Icons ───
function DollarIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>; }
...
function LossIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 14l2-2 2 2"/><path d="M8 6l2 2-2 2"/><path d="M16 6l-2 2 2 2"/><path d="M12 22v-5"/></svg>; }
function UserCheckIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><polyline points="16 11 18 13 22 9"/></svg>; }
...
function PDFIcon() { return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>; }
